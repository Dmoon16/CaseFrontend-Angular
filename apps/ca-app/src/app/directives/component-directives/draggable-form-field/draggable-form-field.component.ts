...
export class DraggableFormFieldComponent implements OnInit, DoCheck {
  ...
  @Input() fieldParticipants: string[] = [];
  @Input() saveModel: any;
  ...
  @Output() toggleReadOnly: EventEmitter<any> = new EventEmitter();
  ...
  checkLineBreak(str: string): number {
    return (str.match(/\n/g) || []).length;
  }
  ...
  changeReadOnlyToogle($event: any): void {
    this.toggleReadOnly.emit($event);
  }
  ...
  startDraging($event: any, wrapper: any, direction1: any, direction2: any) {
    ...
    function draggWatch(e: MouseEvent) {
      e.preventDefault();
  ...
  startChangePosition($event: MouseEvent, wrapper: any) {
    $event.preventDefault();
    this.el.focus();
