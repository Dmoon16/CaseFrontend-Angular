...
@Component({
  selector: 'app-sign-preview',
  templateUrl: './sign-preview.component.html',
  styleUrls: ['./sign-preview.component.css']
})
export class SignPreviewComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  private findMultiTextFields: any[] = [];
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (Object.keys(this.submission || {}).length) {
      this.components.map((component: any) => {
        ...
        if (component.format) {
          component.defaultValue = this.utilsService.doMask(component.defaultValue, component.format);
        }
      });
    }
    this.findTimeFields = [];
    this.findDateFields = [];
    this.findMultiTextFields = [];
    ...
    this.components.forEach((item: any, index: number) => {
      if (item.type === 'textfield') {
        ...
        this.findMultiTextFields.push(temp);
  ...
  private showSelectTimeModal($event: any) {
    ...
    const allMultiTextInputs = document.querySelectorAll('mat-formio-textarea textarea');
    allMultiTextInputs.forEach((input: any, inputIndex) => {
      if (this.findMultiTextFields[inputIndex]?.format) {
        input.addEventListener('keyup', () => {
          var str = input.value;
          input.value = this.utilsService.doMask(str, this.findMultiTextFields[inputIndex]?.format);
        });
      }
    });
    ...
    this.findTimeFields.forEach((item, itemIndex) => {
      allInputs.forEach((input: any, inputIndex) => {
        if (itemIndex === inputIndex && item.time) {
          timeInputs.push(input);
          timeKey.push(item.key);
        } else if (itemIndex === inputIndex && !item.time) {
          if (this.components[item.index]?.format) {
            input.addEventListener('keyup', () => {
              var str = input.value;
              input.value = this.utilsService.doMask(str, this.components[item.index]?.format);
            });
          }
        }
      });
  ...
  convertTime(value: number, timePeriod = false) {
    if (timePeriod) {
      let period;
  ...
  private dateFormat(inputDate: string, format: string) {
