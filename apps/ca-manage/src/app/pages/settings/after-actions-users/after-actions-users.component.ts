...
@Component({
  selector: 'app-after-actions-users',
  templateUrl: './after-actions-users.component.html',
  styleUrls: ['./after-actions-users.component.css']
})
export class AfterActionsUsers extends UnsubscriptionHandler implements OnInit, OnDestroy, AfterViewInit {
  ...
  public createActions() {
    ...
    const oldData: any = this.afterActionList.filter((list: any) => list.case_type == this.assignUserForm.value.case_type);
    const request: any = { assign_users: [...(oldData?.length ? oldData[0].value : []), saveData] };
  ...
  public runRemove(caseTypeId: string) {
    const notification: Notification = this.notificationConnectorService.addNotification({
      title: 'Removing Assign User'
    });

    this.casesService
      .deleteAfterAction(AfterActionType.AssignUsers, caseTypeId)
      .pipe(
        takeUntil(this.unsubscribe$),
        catchError(res => {
          this.saving = false;
  ...
  public getActionList() {
    this.casesService
      .getAfterAction(AfterActionType.AssignUsers)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe((data: any) => {
        this.afterActionList = data.items.length ? data.items.map((item: any) => item) : [];
        this.loading = false;
      });
  }
