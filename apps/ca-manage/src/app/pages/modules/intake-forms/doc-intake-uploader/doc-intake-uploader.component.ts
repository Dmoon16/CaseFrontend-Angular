...
@Component({
  selector: 'app-doc-intake-uploader',
  templateUrl: './doc-intake-uploader.component.html',
  styleUrls: ['./doc-intake-uploader.component.css']
})
export class DocIntakeUploaderComponent implements OnInit {
  ...
  loadExistingFormInfo() {
    const run = () => {
      ...
      return this.intakeFormsService.getIntakeForm(this.form?.asset_id!).subscribe((form: IForm) => {
        this.formModel = form;
        const doc: any = (() => {
          let returnDoc;
  ...
  public handleFileUpload(target: HTMLInputElement): void {
    ...
    const extension: any = fileDataI.name.split('.').pop();
  ...
  public save() {
    const updateFn = () => {
      ...
      this.intakeFormsService.putIntakeForm(request, this.form?.asset_id!).subscribe(
        () => {
          this.loadExistingFormInfo();
        },
        err => {
