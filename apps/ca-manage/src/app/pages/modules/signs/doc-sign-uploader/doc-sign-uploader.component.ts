...
export class DocSignUploaderComponent implements OnInit {
  ...
  constructor(
    ...
    private singsService: SignsService,
  ...
  loadExistingFormInfo() {
    const run = () => {
      ...
      return this.singsService.getFormInfo(this.form?.asset_id!).subscribe((form: IForm) => {
        this.formModel = form;
        const doc: any = (() => {
          let returnDoc;
  ...
  public save() {
    const updateFn = () => {
      ...
      if (request.pages.length == 0) 
        request.pages = null
      this.singsService.updateForm(request, this.form?.asset_id!).subscribe(
        () => {
          this.loadExistingFormInfo();
        },
        err => {
          if (this.singsService.checkRoutes()) {
