...
export class SignPreviewComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  private showSelectTimeModal() {
    // const textFieldsFromForm = document.querySelectorAll('mat-formio-form-field')
    // const dateObjects = []
    //
    // textFieldsFromForm.forEach((formField, index) => {
    //   this.findTimeFields.forEach((item) => {
    //
    //     if (index === item.index && item.time) {
    //       dateObjects.push(formField)
    //     }
    //   })
    // })
