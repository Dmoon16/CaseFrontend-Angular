...
import { IFileToUpload } from '../../pages/modules/media/models/media.model';
 ...
 */
export class FieldsIntakeFormBuilder {
  public form?: any;
  ...
  public async save() {
    ...
    if (this.builderModel.asset_id) {
      this.intakeFormsService.putIntakeForm(this.builderModel, this.builderModel.asset_id).subscribe(
        () => {
  ...
  public loadOpenedInformation() {
    if (this.builderModel.asset_id) {
      ...
      this.subscribers.push(
        this.intakeFormsService.getIntakeForm(this.builderModel.asset_id).subscribe((builderModel: any) => {
  ...
  public calcFieldPosition(): any {
    ...
    for (let i = 0; i < fieldList.length; i++) {
      ...
      top += parseInt(fieldList[i].height, 10);
    }
  ...
  public addField() {
    if (this.newFieldType === 'blank') {
      this.addNewDocumentPage();
    } else if (this.selectedCustomField) {
      ...
      this.editingField.title = this.selectedCustomField.text ?? '';
      this.editingField.defaultValue = this.selectedCustomField.defaultValue ?? '';
      this.editingField.fieldType = this.selectedCustomField.id;
      this.editingField.required = this.selectedCustomField.required ?? false;

      if (this.editingField.fieldType === 'dropdown' || this.editingField.fieldType === 'multidropdown') {
        this.editingField.enum = this.selectedCustomField.enum;
      }
  ...
  public generateIntakeFormsList() {
    // read-only text, upload pdf, upload image, and blank new page. Do similar for the uploader and document
    return [
      { id: 'text', text: 'Text', element: 'Text' },
      { id: 'images', text: 'Upload Image', element: 'Image' },
      { id: 'docs', text: 'Upload Pdf', element: 'Pdf' },
