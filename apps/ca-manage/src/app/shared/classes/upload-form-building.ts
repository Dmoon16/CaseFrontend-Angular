...
import { AuthStatus } from '@manage/interfaces/auth-status.interface';
...
export class UploadFormBuilder {
  ...
  constructor(
    ...
    this.todayDate = this.date.transform(new Date(), 'MM/dd/yyyy');
  ...
  currentIsNotValid() {
    return this.validationDefaultsErrors.indexOf(this.editingField.name + '-f' as string) > -1;
  }
  ...
  public handlePinchMouseMoveEvent(event: any): void {
    ...
    this.prepareFieldValue.format = this.editingField?.format!;
    if (this.editingField?.fieldType === 'text-only') {
  ...
  public dropDownLayoutChanged(event: any) {
    if (this.ifNewField) {
      ...
      this.newProperty.format = this.editingField.format;
  ...
  private checkMediaConvertingInProcess(): void {
    const storageFormModel = localStorage.getItem('sign_model-' + this.assetId);
    if (!storageFormModel) return;
    const formModel = JSON.parse(storageFormModel).sign_model;
      ...
      return this.formsService.getFormInfo(this.assetId).pipe(
        ...
        tap(() =>
          localStorage.setItem(
            'sign_model-' + this.assetId,
            JSON.stringify({ sign_model: { ...this.formModel }, converting_files: [...this.convertingFiles] })
          )
        ),
        switchMap((formModel: IForm) => {
          ...
          if (document[mediaId]?.execution_status === 'SUCCEEDED') {
            ...
            this.docData = {};
            this.refreshModal(false);
            localStorage.removeItem('sign_model-' + this.assetId);
